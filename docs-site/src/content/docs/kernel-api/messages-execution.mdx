...
**App 0.13.0** — additive editor support via [ruff](https://docs.astral.sh/ruff/).
Both are **Python-only** (Q# has its own compiler diagnostics via the QDK
language service) and both degrade gracefully when ruff isn't installed.
...
```json title="lint_result for an unused import"
