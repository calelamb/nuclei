...
QEC engine state belongs to the open project. Switching projects immediately
clears the previous project's selected source, jobs, queries, and canonical
session catalog. Nuclei asks the retiring authenticated client to cancel its
active work in the background, so a slow cancellation cannot delay the new
project and a late frame cannot repopulate the replacement workspace.

If an authenticated data-engine connection closes, the workbench removes the
dead client and shows the disconnect reason. Choose **Retry QEC Data Engine**
to make one new connection attempt. Nuclei does not silently loop or send an
import action through a client created for another project.
