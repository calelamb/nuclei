...
Nuclei is a purpose-built desktop IDE for quantum computing. It combines a Monaco code editor with live circuit visualization, Bloch sphere animations, probability histograms, and an AI teaching assistant called Dirac (powered by Codex API). The primary audience is students taking their first quantum computing course.
...
| AI Assistant | **Codex API** (Haiku for fast, Sonnet for complex) | Dirac — Codex API wrapper with tutor persona, system prompt, context injection, and tool definitions |
...
- Tool definitions passed to Codex's tool use API that let Dirac act on the IDE
...
**Tools (passed to Codex's tool use API):**
