...
function PathSelector({ onSelect }: { onSelect: (path: LearningPath) => void }) {
  ...
  return (
    <div style={{ padding: 16 }}>
      <div style={{ color: colors.accent, fontWeight: 700, fontSize: 16, fontFamily: 'Inter, sans-serif', marginBottom: 12 }}>
        Learning Paths
      </div>
      <div style={{ color: colors.textMuted, fontSize: 12, fontFamily: 'Inter, sans-serif', marginBottom: 16 }}>
        ...
        return (
          ...
          >
            ...
            <div style={{ marginTop: 8, display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ color: colors.textMuted, fontSize: 11, fontFamily: 'Inter, sans-serif' }}>
                {completedCount}/{totalCount}
              </span>
            </div>
            {progress && (
              <div style={{ color: colors.accentLight, fontSize: 11, fontFamily: 'Inter, sans-serif', marginTop: 4 }}>
...
function ModuleCard({ module, index, isActive, isCompleted, onClick }: {
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1 }}>
        <div style={{ color: isActive ? colors.accent : colors.text, fontSize: 12, fontWeight: 500, fontFamily: 'Inter, sans-serif' }}>
          {module.title}
        </div>
        <span style={{
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ flex: 1, overflow: 'auto' }}>
        <div style={{ padding: '8px 12px', borderBottom: `1px solid ${colors.border}` }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ color: colors.accent, fontWeight: 600, fontSize: 13, fontFamily: 'Inter, sans-serif' }}>
              {activePath.title}
            </span>
            <button onClick={exitPath} style={{ background: 'none', border: 'none', color: colors.textMuted, cursor: 'pointer', fontSize: 12 }}>✕</button>
          </div>
        </div>
      ...
      <div style={{ padding: 12, borderTop: `1px solid ${colors.border}` }}>
        <div style={{ color: colors.text, fontSize: 12, fontWeight: 500, fontFamily: 'Inter, sans-serif', marginBottom: 8 }}>
          {module.title}
        </div>
        <div style={{ color: colors.textMuted, fontSize: 11, fontFamily: 'Inter, sans-serif', marginBottom: 10, lineHeight: 1.5 }}>
          {module.description}
        </div>
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          <button onClick={loadDemo} style={{ padding: '4px 10px', background: colors.accent, color: '#fff', border: 'none', borderRadius: 3, cursor: 'pointer', fontSize: 11, fontFamily: 'Inter, sans-serif' }}>
            Load Demo
          </button>
          {module.quizQuestions.length > 0 && (
            <button onClick={() => setShowQuiz(true)} style={{ padding: '4px 10px', background: colors.dirac, color: '#fff', border: 'none', borderRadius: 3, cursor: 'pointer', fontSize: 11, fontFamily: 'Inter, sans-serif' }}>
              Quiz
            </button>
          )}
          {isCompleted && canAdvance && (
            <button onClick={() => setActiveModule(activeModuleIndex + 1)} style={{ padding: '4px 10px', background: colors.bgPanel, color: colors.accent, border: `1px solid ${colors.accent}`, borderRadius: 3, cursor: 'pointer', fontSize: 11, fontFamily: 'Inter, sans-serif' }}>
              Next Module →
            </button>
          )}
          {!isCompleted && (
            <button onClick={() => markModuleComplete(activePath.id, module.id)} style={{ padding: '4px 10px', background: colors.bgPanel, color: colors.textMuted, border: `1px solid ${colors.border}`, borderRadius: 3, cursor: 'pointer', fontSize: 11, fontFamily: 'Inter, sans-serif' }}>
              Mark Complete
            </button>
          )}
        </div>
      ...
      {showQuiz && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 2000, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowQuiz(false)}>
          <div style={{ background: colors.bg, border: `1px solid ${colors.border}`, borderRadius: 8, padding: 20, maxWidth: 420, width: '90%', maxHeight: '80vh', overflow: 'auto' }} onClick={(e) => e.stopPropagation()}>
            <div style={{ color: colors.accent, fontSize: 16, fontWeight: 600, fontFamily: 'Inter, sans-serif', marginBottom: 16 }}>
              Quiz: {module.title}
            </div>
            {module.quizQuestions.map((q, qi) => (
              <div key={q.id} style={{ marginBottom: 16 }}>
                <div style={{ color: colors.text, fontSize: 13, fontFamily: 'Inter, sans-serif', marginBottom: 8 }}>
                  {qi + 1}. {q.question}
                </div>
                {q.options.map((opt, oi) => (
                  <label key={oi} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 0', cursor: 'pointer' }}>
                    ...
                    />
                    <span style={{ color: colors.text, fontSize: 12, fontFamily: 'Inter, sans-serif' }}>{opt}</span>
                  </label>
                ))}
              </div>
            ))}
            <button onClick={handleQuizSubmit} style={{ padding: '8px 20px', background: colors.accent, color: '#fff', border: 'none', borderRadius: 4, cursor: 'pointer', fontSize: 13, fontFamily: 'Inter, sans-serif' }}>
              Submit
            </button>
          </div>
        </div>
  ...
  return (
    <div style={{
      ...
      {activePath ? (
        ...
        <PathSelector onSelect={startPath} />
      )}
    </div>
  );
}
